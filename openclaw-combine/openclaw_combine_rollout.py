...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = OpenClawCombineAPIServer(
            args=args,
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[OpenClawCombineWorker] waiting for combine samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[OpenClawCombineWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_openclaw_combine(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        ...
        print(
            f"[OpenClawCombineWorker] prm_eval_score={extra_metrics['rollout/prm_eval_score']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
