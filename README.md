...
AI 에이전트 런타임 프레임워크 11개를 종합 분석하는 프로젝트입니다. 각 프레임워크의 아키텍처, 보안, 기억 관리, 브라우저 자동화 전략을 심층 비교하며, 밋업 실전 운영 패턴 및 에이전트 결제·신원 프로토콜 분석도 포함합니다.
...
### 1-c. reports/repos/details/nullclaw_report.md (개별)
**주제**: NullClaw (Zig) 심층 분석 (11번째 비교 대상)

**핵심 발견**:
- **생태계 최소 바이너리**: 678 KB 정적 바이너리 — ZeroClaw(8.8 MB)보다 13배 작음. libc 외 의존성 없음 → R15 공급망 공격 표면 제거.
- **WASI 타겟**: `main_wasi.zig` — 11개 프레임워크 중 유일한 WebAssembly 배포 옵션 (R16 에이전트 이식성 표준).
- **보안 Tier 1**: ChaCha20-Poly1305 + Landlock OS 샌드박스 (커널 직접 파일/네트워크 제어). ZeroClaw와 동급.
- **19개 채널**: Signal + Nostr + Matrix 지원 — OpenFang(40) 다음 생태계 2위권. Nostr/Matrix는 10개 프레임워크 중 유일한 탈중앙화 채널.
...
### 1-d. reports/commercial/details/nvidia_nemoclaw_commercial.md (상용화)
...
### 7. reports/meetup/agent_payment_protocol_report.md (결제·신원·상호운용성)
...
│   │       ├── nemoclaw_report.md       # NemoClaw 소스코드 심층 분석
│   │       ├── nullclaw_report.md       # NullClaw (Zig) 심층 분석 (신규)
│   │       └── nvidia_nemoclaw_pre_launch.md  # NemoClaw 출시 전 분석
...
│   ├── meetup/                        # 밋업 실전 운영 패턴 (신규)
│   │   ├── meetup_patterns_report.md     # 실전 운영 패턴 (2-Step Cron, 격리 등)
│   │   └── agent_payment_protocol_report.md  # 에이전트 결제·신원·상호운용성 프로토콜
...
│   │       ├── crew_you_analysis.md          # Crew.you (AI3) 분석 (신규)
│   │       ├── genspark_claw_analysis.md     # Genspark Claw 분석 (신규)
│   │       └── perplexity_personal_computer_analysis.md  # Perplexity PC 분석 (신규)
│   └── deployment/                    # 배포 전략 비교 (4)
├── repos/
│   ├── nemoclaw/                      # NemoClaw 소스코드 (25,650 LOC, Apache 2.0)
│   ├── nullclaw/                      # NullClaw 소스코드 (~249K LOC, MIT) (신규)
...
|------|------|------------------|---------------------|
| **프레임워크** | `reports/repos/` | framework_catalog, framework_cross_analysis, 보안, 기억, 세션, 브라우저 (6) | OpenFang, OpenJarvis, NemoClaw, **NullClaw** (4) |
...
| 차원 | IronClaw | ZeroClaw | OpenClaw | NanoClaw | Nanobot | PicoClaw | TinyClaw | OpenFang | OpenJarvis | NemoClaw | **NullClaw** |
|------|----------|----------|----------|----------|---------|----------|----------|----------|------------|----------|------------|
| **도구 아키텍처** | A | A | S | B | B | B | D | A (60 built-in, MCP, A2A) | A | C (플러그인 전용) | **A** (35+ built-in, vtable, MCP, A2A) |
| **보안** | S | S | B | B | C | C | D | A (16-layer, LLM gates) | B+ | A+ (4-layer, GPU 샌드박스) | **S** (ChaCha20+Landlock OS 샌드박스) |
| **장기기억** | A | A | S | C | B | B | D | B (Phase 1 SQLite) | A | D (위임) | **A** (10 엔진, 벡터+하이브리드) |
| **단기기억(세션)** | A | A | A | B | B | B | C | A (3-layer context) | B+ | C | **A** |
| **브라우저** | D | A | S | B | - | - | - | B (Native CDP, 11 tools) | B | D (위임) | **D** |
| **채널/메신저** | - | - | S | A | A | - | A | S (40 adapters) | A | C | **A** (19채널, Signal+Nostr+Matrix) |
| **자율 실행(24/7)** | B | B | B | B | B | B | B | S (Hands, scheduler) | A | A (GPU 스케줄링) | **A** (heartbeat.zig) |
| **자율성** | B | B | B | B | B | B | B | S | A | B | **B** |
| **MCP 지원** | A | A | A | B | A | A | - | A (bidirectional) | A | B | **A** |
| **멀티에이전트** | B | B | B | S | B | - | A | A (A2A, agent tools) | A | C | **A** (subagent.zig + A2A) |
| **배포 용이성** | C | B | A | A | A | B | B | A (single binary) | A | B | **S** (678KB 정적 바이너리) |
| **고유 혁신** | WASM Zero-Exposure | Soul Snapshot | Plugin 24 hooks | Agent Swarm | 최소 코드 | 병렬 도구 | TUI 대시 | Hands+40채널 | 유연한 A2A | GPU 최적화 샌드박스 | **WASI+Landlock+공급망 표면 최소화** |
...
- **2026-03-17**: NullClaw(Zig) 11번째 프레임워크 추가; 밋업 보고서 2개 신설 (실전 운영 패턴, 에이전트 결제·신원 프로토콜); 상용화 비교 확대 (8개 서비스: Crew.you·Genspark·Perplexity PC 신규); README 구조 개편 (인위적 7/나머지 분리 제거)
