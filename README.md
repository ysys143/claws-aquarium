...
AI 에이전트 런타임 프레임워크 15개를 종합 분석하는 프로젝트입니다. 각 프레임워크의 아키텍처, 보안, 기억 관리, 브라우저 자동화 전략을 심층 비교하며, 커뮤니티 유즈케이스·밋업 실전 운영 패턴·에이전트 결제·신원 프로토콜 분석도 포함합니다.
...
| 14 | **AgentScope** (Alibaba) | Python | agentscope-ai/agentscope | 42K | Apache-2.0 | OTel-first 멀티에이전트 플랫폼; 내장 프롬프트 튜너, A2A+MCP, CoPaw·ReMe 모체 |
| 15 | **CoPaw** | Python | agentscope-ai/CoPaw | 84K | Apache-2.0 | 14채널 개인 비서(생태계 최다 채널); 3-tuple QueueKey 격리, Playwright 브라우저, Skills 스캐닝 |
...
| # | 이름 | 언어 | 저장소 | 핵심 특징 |
|---|------|------|--------|----------|
| 1 | **MetaClaw** | Python | aiming-lab/MetaClaw | 7개+ Claw 백엔드 교체 가능한 메타 레이어; GRPO RL, MadMax 유휴-창 스케줄링, 실패 기반 스킬 진화(R35-R37) |
...
│   ├── agentscope/                    # AgentScope (Alibaba, ~42K LOC, Apache 2.0)
...
| 차원 | IronClaw | ZeroClaw | OpenClaw | NanoClaw | Nanobot | PicoClaw | TinyClaw | OpenFang | OpenJarvis | NemoClaw | NullClaw | Hermes | GoClaw | AgentScope | CoPaw |
|------|----------|----------|----------|----------|---------|----------|----------|----------|------------|----------|----------|--------|--------|------------|-------|
| **도구 아키텍처** | A | A | S | B | B | B | D | A (60 built-in, MCP, A2A) | A | C (플러그인 전용) | A (35+ built-in, vtable, MCP, A2A) | B (Skills 4단계) | B+ (MCP+20+LLM+KG) | A (MCP>=1.13, A2A) | A (Skills+browser) |
| **보안** | S | S | B | B | C | C | D | A (16-layer, LLM gates) | B+ | A+ (4-layer, GPU 샌드박스) | S (ChaCha20+Landlock) | B+ (Tirith+Memory Injection+Skills Trust) | A+ (Docker 3축+AES+Tailscale) | C (hooks only) | B (tool_guard+skill_scanner) |
| **장기기억** | A | A | S | C | B | B | D | B (Phase 1 SQLite) | A | D (위임) | A (10 엔진, 벡터+하이브리드) | A (Frozen Snapshot+char 예산) | B (임베딩+PostgreSQL) | A (SQLAlchemy+mem0+Redis) | B (ReMe 통합) |
| **단기기억(세션)** | A | A | A | B | B | B | C | A (3-layer context) | B+ | C | A | A (prefix cache 보존) | B | B | B |
| **브라우저** | D | A | S | B | - | - | - | B (Native CDP, 11 tools) | B | D (위임) | D | D | B (go-rod) | D | A (Playwright 3,460줄) |
| **채널/메신저** | - | - | S | A | A | - | A | S (40 adapters) | A | C | A (19채널, Signal+Nostr+Matrix) | A (6종 메시징) | A (7채널) | D (채널 없음) | S (14채널, 생태계 최다) |
| **자율 실행(24/7)** | B | B | B | B | B | B | B | S (Hands, scheduler) | A | A (GPU 스케줄링) | A (heartbeat.zig) | B | A (cron+scheduler) | B | A (apscheduler) |
| **MCP 지원** | A | A | A | B | A | A | - | A (bidirectional) | A | B | A | A | A | S (>=1.13 필수) | A (AgentScope 통해) |
| **멀티에이전트** | B | B | B | S | B | - | A | A (A2A, agent tools) | A | C | A (subagent.zig + A2A) | B (Bounded Tree, depth=2) | A (Teams+blocked_by DAG) | A (A2A+nacos) | B |
| **배포 용이성** | C | B | A | A | A | B | B | A (single binary) | A | B | S (678KB 정적 바이너리) | B | S (단일 바이너리+Docker Compose) | B (pip install) | B (pip+Docker) |
| **고유 혁신** | WASM Zero-Exposure | Soul Snapshot | Plugin 24 hooks | Agent Swarm | 최소 코드 | 병렬 도구 | TUI 대시 | Hands+40채널 | 유연한 A2A | GPU 최적화 샌드박스 | WASI+Landlock+공급망 표면 최소화 | Frozen Snapshot+Tirith Pre-Exec | 3축 Docker 샌드박스+Tailscale VPN | OTel-first+프롬프트 튜너+모델 선택기 | 14채널+3-tuple QueueKey 격리 |
...
Tier A: IronClaw       -- 보안+기억 최강. 진입장벽이 약점
        ...
        AgentScope     -- OTel-first+프롬프트 튜너+A2A. 채널 없음(CoPaw 의존)
...
| **AgentScope** | OTel-first 멀티에이전트 플랫폼 | OTel 코어 내장(R40), 프롬프트 튜너+모델 선택기(R41), A2A+MCP>=1.13, arXiv 논문 | 채널 없음(CoPaw 의존), Alibaba 종속 의존성 |
| **CoPaw** | 14채널 Skills 기반 개인 비서 | 채널 최다(14개, R42 QueueKey 격리), Playwright 브라우저(3,460줄), 3단계 보안 스캐닝 | AgentScope 버전 고정(==1.0.18), 중국 채널 편중 |
...
- **2026-03-30**: GoClaw(Go, 1.4k stars)·AgentScope(Alibaba, 22k stars)·CoPaw(13.6k stars) repos/ 등록 (14th~16th); R38-R42 신규 패턴 (3축 샌드박스, Tailscale VPN, OTel-first, 프롬프트 튜너, 3-tuple QueueKey); 평가 매트릭스 15개 프레임워크로 확장; MetaClaw README 분석 대상 추가; Claude Code 분석 테이블 제외
