...
Claw 에이전트 런타임의 8가지 주요 구현체와 OpenFang Agent OS를 종합 분석하는 프로젝트입니다. 각 프레임워크의 아키텍처, 보안, 기억 관리, 브라우저 자동화 전략을 심층 비교합니다.
...
### 8대 Claw 프레임워크 + OpenFang Agent OS
...
| 8 | **Moltbook** | TypeScript | (분석 대상 소스) | 32 REST 엔드포인트, 최소 프로토타이핑용 |
| 9 | **OpenFang** | Rust | RightNow-AI/openfang | Agent OS: 24/7 자율 실행, 7 Hands, 40 채널, 16-layer 보안, ~32MB 단일 바이너리 |

### 연구 자동화 도구 (repos_research/)

기존 에이전트 런타임과 별도로, **연구 자동화 특화 도구** 2개를 분석한다. idea3(AI Research Agent), idea4(Lab AI Agent) 설계에 직접 참조된다.
...
**활용**: idea3(AI Research Agent), idea4(Lab AI Agent) 설계 참조, 연구 자동화 패턴 재사용

### 1. openfang_report.md
...
### 2. openclaw_ecosystem_report.md
**주제**: 7대 프레임워크의 기본 정보, 특징, 배포 방식 종합 정리
...
### 5. browser_actions_report.md & memory_architecture_report.md
...
│   ├── idea3.md                       # AI Research Agent 아키텍처 (X→Obsidian→CLAUDE.md 파이프라인 포함)
│   └── idea4.md                       # Lab AI Agent 설계 (랩 도서관→자동화→연구보조 3단계)
├── reports/
│   ├── research_tools_report.md        # 연구 자동화 도구 분석 (DeepInnovator & Autoresearch)
│   ├── openfang_report.md              # OpenFang Agent OS 심층 분석 (9번째 대상)
...
| 차원 | IronClaw | ZeroClaw | OpenClaw | NanoClaw | Nanobot | PicoClaw | TinyClaw | **OpenFang** |
|------|----------|----------|----------|----------|---------|----------|----------|------------|
| **보안** | S | S | B | B | C | C | D | **A** (16-layer, LLM gates) |
| **장기기억** | A | A | S | C | B | B | D | **B** (Phase 1 SQLite) |
| **단기기억(세션)** | A | A | A | B | B | B | C | **A** (3-layer context) |
| **도구 아키텍처** | A | A | S | B | B | B | D | **A** (60 built-in, MCP, A2A) |
| **브라우저** | D | A | S | B | - | - | - | **B** (Native CDP, 11 tools) |
| **MCP 지원** | A | A | A | B | A | A | - | **A** (bidirectional, 25 pre-config) |
| **채널/메신저** | - | - | S | A | A | - | A | **S** (40 adapters, model override) |
| **자율 실행(24/7)** | B | B | B | B | B | B | B | **S** (Hands, 3-mode scheduler) |
| **멀티에이전트** | B | B | B | S | B | - | A | **A** (A2A protocol, agent tools) |
| **배포 용이성** | C | B | A | A | A | B | B | **A** (single binary, one install) |
| **고유 혁신** | WASM Zero-Exposure | Soul Snapshot | Plugin 24 hooks | Agent Swarm | 최소 코드 | 병렬 도구 | TUI 대시 | **Hands+40채널** |

S=최고, A=우수, B=보통, C=약함, D=없음/최소, -=해당없음
...
| 용도 | 1순위 | 2순위 | 이유 |
|------|-------|-------|------|
| **24시간 메신저 에이전트** | OpenClaw | NanoClaw | 메신저+기억+도구 올인원 |
...
| **멀티에이전트 오케스트레이션** | NanoClaw | TinyClaw | 스웜/팀 네이티브 지원 |
...
| 파일 | 주제 | 핵심 |
|------|------|------|
| **idea3.md** | AI Research Agent | 논문 검색·분석·집필 파이프라인. X→Obsidian→CLAUDE.md 지식 자동 수집 구조 포함 |
| **idea4.md** | Lab AI Agent | 연구랩 10명을 위한 지식 통합 에이전트. 3단계 로드맵(도서관→자동화→연구보조) |
