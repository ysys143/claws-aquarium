...
> **조사 일자**: 2026-03-05
> **조사 방법**: 7개 scientist 에이전트가 각 레포의 브라우저 자동화 및 도구/액션 소스코드를 병렬 심층 분석
...
7개 구현체의 소스코드를 분석한 결과, **브라우저 자동화는 4개만 보유**하고, **도구 아키텍처는 6가지 유형**으로 분류된다.
...
3. **MCP가 사실상의 표준 확장 프로토콜이 되었다.** 7개 중 5개(IronClaw, ZeroClaw, PicoClaw, Nanobot, OpenClaw)가 MCP 서버 통합을 지원한다. 직접 도구를 구현하지 않고도 외부 MCP 서버를 래핑하여 도구로 등록하는 패턴이 공통적이다.
