...
> **조사 일자**: 2026-03-05 (최초), 2026-03-14 (OpenJarvis 추가)
> **조사 방법**: 7개 scientist 에이전트가 각 레포의 브라우저 자동화 및 도구/액션 소스코드를 병렬 심층 분석; OpenJarvis는 별도 분석 후 추가
...
10개 구현체의 소스코드를 분석한 결과, **브라우저 자동화는 4개만 보유**하고, **도구 아키텍처는 7가지 유형**으로 분류된다.
...
3. **MCP가 사실상의 표준 확장 프로토콜이 되었다.** 10개 중 6개(IronClaw, ZeroClaw, PicoClaw, Nanobot, OpenClaw, OpenJarvis)가 MCP 서버 통합을 지원한다. 직접 도구를 구현하지 않고도 외부 MCP 서버를 래핑하여 도구로 등록하는 패턴이 공통적이다.
...
TinyClaw       PicoClaw       NanoClaw       ZeroClaw      IronClaw
               Nanobot        OpenClaw(Docker)
               OpenClaw       OpenFang(WASM)
               OpenJarvis
```

IronClaw의 WASM+프록시 이중 격리가 가장 안전하지만, 도구 개발 복잡도도 가장 높다 (WIT 인터페이스 + Component Model 이해 필요). OpenJarvis는 격리 없이 프로세스 내 직접 실행 — 에이전트 위임(openhands 통합)으로 간접 격리를 대신한다.
...
| **Tier 2** (NanoClaw, OpenClaw, OpenFang) | Docker 컨테이너 / WASM Dual Metering | stdin/환경변수 / Ed25519 서명 | 도구 허용목록 / 18종 Capability |
| **Tier 3** (Nanobot, PicoClaw) | 프로세스 내 | 평문 | deny 패턴 |
| **Tier 4** (TinyClaw, OpenJarvis) | 없음/최소 | 없음/최소 | Loop Guard (OpenJarvis만) |

OpenJarvis는 도구 격리나 크레덴셜 보호 메커니즘이 없으나, Loop Guard(반복 tool call 탐지+컨텍스트 압축)로 무한 루프를 방지하는 최소한의 런타임 안전장치는 갖추고 있다.
