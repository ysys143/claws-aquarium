# 기억 아키텍처 비교 분석: 8개 Claw 프레임워크의 중기/장기 기억 구현

> **조사 일자**: 2026-03-05
> **조사 방법**: 6개 scientist 에이전트가 각 레포의 실제 소스코드를 병렬 심층 분석 (Tier 1: 3x sonnet, Tier 2: 2x haiku, Tier 3: 1x haiku)
...
8개 Claw 구현체의 중기/장기 기억 시스템을 코드 수준에서 분석한 결과, **3단계 기억 성숙도**로 분류된다:
...
| **Tier 3: Delegation/None** | NanoClaw, TinyClaw, Moltbook | 자체 기억 없음, 외부 시스템에 위임 또는 기억 미구현 |
...
| 차원 | NanoClaw | TinyClaw | Moltbook |
|------|----------|----------|----------|
| **기억 전략** | CLAUDE.md 위임 | write-only 채팅 아카이브 | 없음 (소셜 DB) |
| **중기기억** | 그룹별 CLAUDE.md | 동적 AGENTS.md 생성 | 없음 |
| **장기기억** | 대화 아카이브 (비자동 로딩) | 채팅 히스토리 (비자동 로딩) | PostgreSQL (posts/comments) |
...
### 4.8 Moltbook (Tier 3 -- 에이전트 기억 없음)

- **소셜 DB만 존재** (`scripts/schema.sql`): `agents`, `submolts`, `posts`, `comments`, `votes` 테이블
- **에이전트 프로필**: `agents` 테이블에 karma, stats 등 사회적 메타데이터
- REST API 기반 외부 에이전트 클라이언트 대상 -- 자체 기억 시스템 불필요
...
| **Moltbook** | PostgreSQL | REST API | 없음 | 없음 | 없음 | 없음 |
...
| **C. 없음 (No Promotion)** | NanoClaw, TinyClaw, Moltbook | 장기 저장소 -> 중기 로딩 경로 자체가 부재 |
...
| **Moltbook** | [X] | [X] | [O] | N/A | N/A | N/A | 0/6 |
