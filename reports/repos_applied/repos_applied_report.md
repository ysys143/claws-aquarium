...
2개 프로젝트 모두 "설계됐으나 구현되지 않은" 부분을 가진다:
...
ClawPort의 테스트 성숙도가 ClawWork를 압도한다. ClawWork는 경제 엔진(`EconomicTracker`)이 복잡한 상태 관리를 담당하는데 테스트가 전혀 없다는 점이 특이하다.
