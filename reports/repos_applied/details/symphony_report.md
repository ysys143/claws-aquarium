...
> **선행 보고서**: repos_applied_report.md (ClawWork, ClawPort 분석)
...
2. **SKILL.md는 생태계를 초월한 de facto 인터페이스 컨벤션이 됐다.** NanoClaw(원조) → ClawWork(주입) → Symphony(YAML 공식화)의 진화 경로가 확인된다. OpenAI가 독립적으로 동일한 컨벤션을 채택했다는 사실이 이 패턴의 수렴 압력을 입증한다.
...
두 형식 모두 "YAML 헤더 + 마크다운 본문 = 에이전트 정책 파일" 구조를 공유한다. 이 패턴이 단일 컨벤션으로 수렴하고 있는가? OpenAI의 독립적 채택을 포함하면 이 패턴의 확산이 생태계 전체 표준 수립 과정인가?
