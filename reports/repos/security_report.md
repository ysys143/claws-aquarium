...
| 계층 | 구현체 | 특징 |
|------|--------|------|
| **Tier 1: Defense-in-Depth** | IronClaw, ZeroClaw, **OpenFang** | 암호화 볼트 + WASM/Docker 이중 샌드박스 + 다층 인젝션 방어 + 비용 제한. IronClaw·ZeroClaw는 진짜 HITL(사람이 코드 레벨에서 강제). **OpenFang**: WASM Dual Metering + 18종 Capability + Taint Tracking + Keyring으로 실행 감시는 최상급이나 승인 게이트가 LLM 프롬프트 기반 — HITL 부재가 Tier 1 상한선 |
