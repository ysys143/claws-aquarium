...
AgentScope(github.com/agentscope-ai/agentscope, 22k stars)는 Alibaba Tongyi Lab의 연구 기반 멀티에이전트 플랫폼이다(arXiv:2402.14034). CoPaw의 직접 런타임 기반이며, 다음 특징이 CoPaw에 그대로 상속된다:

| AgentScope 특성 | CoPaw에서의 역할 |
|----------------|----------------|
| OpenTelemetry 코어 내장 (>=1.39.0) | CoPaw의 모든 LLM 호출이 자동 분산 트레이싱 |
| MCP >=1.13 필수 의존성 | CoPaw의 외부 도구 연결 표준 |
| A2A 프로토콜 (a2a-sdk + nacos) | 분산 에이전트 간 서비스 디스커버리 |
| SQLAlchemy + Redis 워킹 메모리 | CoPaw 메모리 하위 레이어 |
| ReAct 에이전트 내장 | CoPaw의 react_agent.py 기반 |
| hooks/ 라이프사이클 | CoPaw의 agents/hooks/ 로 확장 |

AgentScope 자체는 메시징 채널을 제공하지 않는다 — 이것이 CoPaw가 존재하는 이유다.

### 3.2 agentscope-ai 생태계 구조

```
agentscope (플랫폼 코어, 22k stars)
  ├── agentscope-runtime (1.1.2b2) — 확장 런타임
  ├── CoPaw (13.6k stars) — 채널+Skills+브라우저 레이어
  └── ReMe (reme-ai 0.3.1.6) — 메모리 특화 파생체
```

CoPaw의 `pyproject.toml`이 세 개를 모두 직접 pin:
```toml
agentscope==1.0.18
agentscope-runtime==1.1.2b2
reme-ai==0.3.1.6
```

### 3.3 CoPaw가 AgentScope 위에 추가하는 것

CoPaw는 AgentScope가 의도적으로 제외한 "채널 + 사용자 인터페이스 레이어"를 완성한다:

| 레이어 | AgentScope | CoPaw 추가분 |
|--------|-----------|-------------|
| 에이전트 런타임 | AgentBase, ReAct, pipeline | routing_chat_model.py (모델 라우팅) |
| 채널 | 없음 | 14개 채널 (84K LOC의 ~17%) |
| 메시지 큐 | 없음 | UnifiedQueueManager (3-tuple 키) |
| 스킬 | 없음 | skills_manager.py (2,557L) + skills_hub.py (1,671L) |
| 브라우저 | 없음 | browser_control.py (3,460L, Playwright) |
| 보안 스캐닝 | 없음 | tool_guard + file_access_guard + skill_scanner |
| 스케줄링 | 없음 | apscheduler 기반 cron |
| 웹 콘솔 | 없음 | React 빌드 산출물 내장 |

### 3.4 버전 고정의 함의

`agentscope==1.0.18` 핀은 AgentScope 업스트림 변경으로부터 CoPaw를 격리한다. 이는 안정성은 확보하지만, AgentScope의 신기능(새 MCP 버전, 새 A2A 구현)을 즉시 채택하지 못하는 트레이드오프다. agentscope-ai 조직이 두 레포를 함께 관리하므로 실질적 충돌 위험은 낮다.
