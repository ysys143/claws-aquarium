...
> **조사 일자**: 2026-03-05 (OpenFang/OpenJarvis/NemoClaw 추가: 2026-03-17)
> **조사 방법**: 5개 scientist 에이전트가 각 레포의 실제 소스코드를 병렬 심층 분석 (Tier 1: 3x sonnet, Tier 2: 2x haiku, Tier 3: 1x haiku); OpenFang/OpenJarvis/NemoClaw 별도 추가 분석
...
10개 Claw 구현체의 중기/장기 기억 시스템을 코드 수준에서 분석한 결과, **3단계 기억 성숙도**로 분류된다:

| 성숙도 | 구현체 | 핵심 특성 |
|--------|--------|-----------|
| **Tier 1: Full Memory Stack** | IronClaw, OpenClaw, ZeroClaw, **OpenFang**, **OpenJarvis** | 벡터 임베딩 + FTS + 하이브리드 검색 + 자동 수명주기 관리 |
| **Tier 2: Structured Markdown** | Nanobot, PicoClaw | 파일 기반 기억 + LLM/규칙 기반 정리, 검색 API 없음 |
| **Tier 3: Delegation/None** | NanoClaw, TinyClaw, **NemoClaw** | 자체 기억 없음, 외부 시스템에 위임 또는 기억 미구현 |
...
| 차원 | IronClaw | OpenClaw | ZeroClaw | OpenFang | OpenJarvis |
|------|----------|----------|----------|----------|------------|
| **저장 백엔드** | PostgreSQL (pgvector) / libSQL | SQLite (sqlite-vec) + LanceDB | SQLite (FTS5 + vector BLOB) | SQLite BLOB (v7 schema, 11 tables) | SQLite/FTS5 + FAISS + ColBERTv2 + BM25 + Hybrid RRF |
| **임베딩 프로바이더** | 4개 (OpenAI, NEAR, Ollama, Mock) | 6개 (openai, gemini, voyage, mistral, ollama, local) | 3개 (openai, openrouter, custom) | 설정 가능 | ColBERTv2 late-interaction 포함 |
| **하이브리드 검색** | RRF (k=60) | Weighted merge -> Temporal decay -> MMR | Linear weighted (0.7v + 0.3k) | Knowledge Graph + importance scoring | Hybrid RRF (5 backends) |
| **수명주기 관리** | 30일 보존, 12h hygiene | Atomic reindex, stale pruning | 12h hygiene, 5-stage pipeline | 자동 (importance scoring 기반) | CLI-driven indexing, 민감 파일 자동 필터링 |
| **자아 보호** | 4개 파일 쓰기 금지 | untrusted-data 면책 선언 | autosave 키 블랙리스트 | Soul Snapshot 24/7 지속성 | trace-driven learning feedback |
...
| 차원 | NanoClaw | TinyClaw | NemoClaw |
|------|----------|----------|----------|
| **기억 전략** | CLAUDE.md 위임 | write-only 채팅 아카이브 | OpenClaw 호스트에 위임 |
| **중기기억** | 그룹별 CLAUDE.md | 동적 AGENTS.md 생성 | OpenClaw의 컨텍스트 관리에 의존 |
| **장기기억** | 대화 아카이브 (비자동 로딩) | 채팅 히스토리 (비자동 로딩) | 런 상태 (~/.nemoclaw/state/runs/) + blueprint 캐시만 |
...
### 4.8 OpenFang (Tier 1 — Soul Snapshot + Knowledge Graph)

#### 중기기억

**3-layer 컨텍스트 윈도우**: 시스템 프롬프트(12-section 빌더) + 대화 히스토리 + 도구 결과의 3층 구조. 채널별 모델 오버라이드 지원으로 채널 유형에 따라 다른 컨텍스트 전략 적용 가능.

**Soul Snapshot + 24/7 지속성**: ZeroClaw의 Soul Snapshot 패턴과 유사하지만, OpenFang은 24/7 자율 실행을 전제로 설계. 재시작 시 Soul Snapshot에서 자동 복원되어 에이전트 정체성 연속성 유지.

**이중 주입 패턴 (Dual Injection)**: MEMORY.md/Soul 파일은 시스템 프롬프트에 항상 주입(중기) + Knowledge Graph 검색 결과는 동적 주입(장기→중기 승격). Tier 1 전체에 공통된 패턴이나, OpenFang은 Capability 시스템과 통합되어 기억 접근에도 권한 부여.

#### 장기기억

**저장 백엔드**: SQLite BLOB, v7 스키마, 11개 테이블. importance scoring 기반 자동 수명주기 관리.

**Knowledge Graph**: 엔티티 추출 + importance scoring으로 장기 사실 구조화. 단순 벡터 유사도 검색 대신 그래프 구조로 관계 정보 보존.

**쓰기 경로**: 자동 importance scoring → 중요도 높은 기억만 장기 저장소로 승격. 에이전트가 수동으로 `memory_write`를 호출할 필요 없이 자동 필터링.

**고유 특성**: Hands 시스템(플러그인 생태계) + FangHub 마켓플레이스와 통합 — 외부 플러그인도 기억 시스템에 접근 가능. MCP 양방향 지원으로 외부 메모리 서비스 연동.

---

### 4.9 OpenJarvis (Tier 1 — ColBERTv2 + Multi-Backend)

#### 중기기억

**AgentContext 대화 히스토리**: 인터랙션별 AgentContext로 대화 히스토리 관리. 자동 기억 주입(context_from_memory)으로 top-k 검색 결과를 시스템 메시지에 prepend.

**5개 백엔드 지원**: SQLite/FTS5, FAISS, ColBERTv2, BM25, Hybrid RRF — 런타임에 백엔드 조합 선택 가능. ZeroClaw의 교체 가능 백엔드 패턴과 유사하나, 선택지가 더 다양.

**자동 기억 주입**: `context_from_memory` 플래그 활성화 시 매 인터랙션 전 top-k 기억 검색 → 시스템 메시지로 자동 주입. OpenClaw의 LanceDB auto-recall과 동일 패턴.

#### 장기기억

**청크 문서 저장**: paragraph-aware 분할 + 소스 attribution 보존. 출처 추적이 가능해 기억의 신뢰도 평가 가능.

**ColBERTv2 late-interaction 검색**: 쿼리와 문서 토큰 간 fine-grained 매칭. 기존 dense retrieval 대비 긴 문서에서 더 정밀한 검색.

**CLI 기반 인덱싱**: `jarvis memory index` 명령으로 문서 인덱싱. 민감 파일(.env, *.key 등) 자동 필터링으로 기억 오염 방지.

**쓰기 경로**: CLI-driven + trace-driven learning feedback. 에이전트 실행 트레이스를 분석해 유용한 패턴을 장기 기억으로 자동 승격하는 학습 피드백 루프.

**고유 특성**: 9개 에이전트 타입이 각각 다른 기억 전략 사용 — orchestrator는 전체 컨텍스트, simple은 최소 컨텍스트. ClaudeCodeAgent는 subprocess에 위임.

---

### 4.10 NemoClaw (Tier 3 — OpenClaw 위임)

#### 중기기억

- **OpenClaw 의존**: NemoClaw 자체 중기기억 없음. 샌드박스 내에서 OpenClaw의 컨텍스트 관리에 전적으로 의존.
- **blueprint 캐시**: 환경 설정(blueprint) 버전 캐싱. 기억이 아닌 환경 재현을 위한 스냅샷.

#### 장기기억

- **런 상태 지속성** (`~/.nemoclaw/state/runs/`): 샌드박스 실행 이력. 마이그레이션 스냅샷(tar 아카이브)으로 런 상태 이식 가능.
- **blueprint 캐시만**: 자체 knowledge 저장소 없음. 보안/격리에 집중하는 단일 책임 원칙의 결과.
- **핵심 제한**: 기억은 NemoClaw의 관심사가 아님. "OpenClaw 호스트의 메모리 시스템 안에서 샌드박스로 실행"이 설계 철학. 메모리 기능을 원하면 OpenClaw 호스트 설정에서 처리.
...
| **OpenFang** | 3-layer (system/conversation/tool results) | System prompt (12-section builder) + 채널별 모델 오버라이드 | 없음 | 없음 | Soul Snapshot 자동 복원 + 이중 주입 |
| **OpenJarvis** | AgentContext 대화 히스토리 | System prompt + context_from_memory 자동 주입 | 없음 | 없음 | top-k 자동 검색 주입 |
| **NemoClaw** | OpenClaw 의존 | OpenClaw 컨텍스트 관리 위임 | 없음 | 없음 | 없음 (OpenClaw에 위임) |
...
| **OpenFang** | SQLite BLOB (v7, 11 tables) + Knowledge Graph | Knowledge Graph + importance scoring | 없음 (그래프 구조) | 설정 가능 | 자동 (importance 기반) | Soul Snapshot 자동 복원 + 이중 주입 |
| **OpenJarvis** | SQLite/FTS5 + FAISS + ColBERTv2 + BM25 | Hybrid RRF (5 backends) | Hybrid RRF | ColBERTv2 late-interaction | CLI-driven, 민감 파일 자동 필터 | context_from_memory 자동 주입 |
| **NemoClaw** | 런 상태 (~/.nemoclaw/state/runs/) + blueprint 캐시 | 없음 | 없음 | 없음 | 없음 | 없음 |
...
| **OpenFang** | [O] memory_write (importance scoring 자동) | [O] importance 기반 자동 승격 | [X] | [O] Soul Snapshot | [X] |
| **OpenJarvis** | [X] (CLI-driven: jarvis memory index) | [O] trace-driven learning feedback | [X] | [X] | [X] |
| **NemoClaw** | [X] | [X] | [O] 마이그레이션 스냅샷 (tar archive) | [X] | [X] |
...
| **OpenFang** | [O] Soul Snapshot | [O] Knowledge Graph | [O] | [O] Capability-gated | [WARN] DB 접근 | [O] Soul Snapshot 복원 | 5/6 |
| **OpenJarvis** | [O] | [O] ColBERTv2 | [O] FAISS 인덱스 | [WARN] trace-driven soft | [O] CLI 직접 편집 | [WARN] CLI re-index 필요 | 4/6 |
| **NemoClaw** | [X] | [X] | [O] | [X] | [X] | [WARN] blueprint 캐시만 | 1/6 |
...
*본 보고서는 session_context_report.md (단기기억), security_report.md (보안), browser_actions_report.md (도구)와 함께 10개 Claw 프레임워크의 4번째 교차 분석이다. (OpenFang, OpenJarvis, NemoClaw 2026-03-17 추가)*

---

## 8. Meetup 실전 패턴 (2026-03 추가)

### 8.1 파일시스템 교리 엔진 패턴

**소스**: 허예찬 발표 (10_허예찬_OMX_OMC_Claw운영생태계)

**핵심 개념**: "기억이 동작 규칙이 된다"

기존 기억 시스템이 "무엇을 기억하는가"에 집중했다면, 파일시스템 교리 엔진은 기억 자체를 에이전트의 행동 규칙으로 승격시키는 패턴.
...
MEMORY.md
  → 실제 내용 없음
  → 포인터(지도)만 포함
  → "이 정보는 agents/project-context.md에 있다"
...
**기존 MEMORY.md 이중 주입 패턴(§6)과 대비**:

| 항목 | §6 이중 주입 패턴 | 파일시스템 교리 엔진 |
|------|----------------|------------------|
| **MEMORY.md 역할** | 기억 내용 직접 저장 | 포인터(지도)만 저장 |
| **행동 규칙 위치** | 시스템 프롬프트 별도 | agents.md에 기억→규칙 승격 |
| **검색 방식** | 벡터 DB + 파일 검색 | 코딩 에이전트 bash/grep 강점 활용 |
| **규모 한계** | 벡터 DB 필요 | 10만 줄 이상도 bash 검색으로 처리 |
| **사람 가독성** | 중간 | 높음 (파일 직접 읽기 가능) |

**실전 결과 (OMC/OMX 사례)**:
- 최근 100개 PR 중 80% 이상이 AI 에이전트(개발가제) 생성
- 보고 없는 실행 = 의미 없음 / 실행 없는 보고 = 의미 없음 규칙이 agents.md에 명시
- 상태를 읽었으면 반드시 다음 액션까지 수행하는 행동 원칙 강제

---

### 8.2 3계층 계층적 메모리 패턴

**소스**: 정세민 발표 (16_정세민_Ultraworker_AI_Orchestration, Sionic AI)

**핵심 개념**: 검색 방식의 한계에 따라 저장소를 계층적으로 쌓는 구조

```
[Layer 1: 파일시스템]
  - 직접 키워드 매칭 (grep, bash)
  - 빠르고 간단
  - 한계: 업무 맥락 복잡 시 키워드 매칭 실패

[Layer 2: 벡터 DB (BM25 + Quadrant)]
  - BM25 키워드 검색 + 의미 벡터 검색 하이브리드
  - 전체 슬랙 기록 저장 → 과거 채널 기록 기반 답변
  - 한계: 키워드가 완전히 다른 관계형 맥락은 탐지 어려움

[Layer 3: 그래프 DB]
  - 벡터로도 잡히지 않는 관계형 맥락 연결
  - 엔티티 간 관계 추론
  - 가장 복잡하나 가장 풍부한 맥락 제공
```

**6W 온톨로지 랭킹**: 육하원칙(언제/어디서/누가/무엇을/왜/어떻게) 기반 룰베이스 온톨로지로 맥락 랭킹 알고리즘 구성.

**실전 사례**: 오래된 슬랙 채널 기록을 스캔하여 러스트 서비스 장애 원인(캐시 로즈 이슈, 마이그레이션 이슈 등 3가지)을 자동으로 파악하고 완벽히 답변.

**기존 Tier 1 프레임워크와 비교**:

| 항목 | OpenClaw Tier 1 | 정세민 3계층 패턴 |
|------|----------------|----------------|
| **검색 백엔드** | SQLite + LanceDB 하이브리드 | 파일시스템 → BM25+Quadrant → 그래프DB |
| **맥락 랭킹** | Temporal decay + MMR | 6W 온톨로지 기반 룰베이스 |
| **관계 추론** | 없음 | 그래프 DB (3층) |
| **슬랙 통합** | 없음 | 전체 슬랙 기록 벡터화 |
| **HITL 연동** | 없음 | 슬랙 BlockKit 승인 UI |

---

### 8.3 탈중앙화 메모리 레이어 패턴 (Unibase)

**소스**: Zoe Chen 발표 (14_Zoe_Chen_Unibase_Extending_OpenClaw)

**핵심 개념**: 에이전트 기억을 단일 디바이스가 아닌 탈중앙화 네트워크에 저장하여, 기기 간 마이그레이션과 자기 진화를 가능하게 하는 인프라.
...
[탈중앙화 메모리 (Unibase)]
  Mac Mini ─┐
  PC        ├── Unibase 탈중앙화 레이어 ── 에이전트 기억
  스마트폰 ─┘
  기기 교체 시 → 기억 마이그레이션 자동
```

**3가지 핵심 기능**:
1. **크로스 디바이스 기억 마이그레이션**: 어떤 디바이스에서든 동일한 기억 접근
2. **자기 진화(Self-Evolving)**: 매일 축적되는 컨텍스트와 지식 기반으로 사용자 지시 없이 자율 학습
3. **AIP(Agent Interoperability Profile) 연동**: 에이전트 간 협력 시 기억 공유 및 신원 확인

**설치 방식**: GitHub 링크를 OpenClaw에 전달 → 에이전트가 자동으로 메모리 스킬 설치.

**기존 R9 Sleep Consolidation Loop(§6)과 대비**:

| 항목 | R9 Sleep Consolidation | Unibase 탈중앙화 |
|------|----------------------|----------------|
| **기억 위치** | 단일 서버 (로컬/클라우드) | 탈중앙화 네트워크 |
| **기기 교체** | 기억 이전 수동 필요 | 자동 마이그레이션 |
| **통합 주기** | 30분 슬립 사이클 | 상시 + 자율 학습 |
| **에이전트 간 공유** | 없음 | AIP 프로토콜로 가능 |
| **한계** | 단일 기기 의존 | 탈중앙화 인프라 의존 |

---

### 8.4 Subagent Context 격리 패턴

**소스**: 윤주운 발표 (09_윤주운_Arbiter_FLOCK_워크플로우, Sionic AI)

**문제**: OpenClaw의 서브에이전트는 부모 에이전트의 모델·메모리·시스템 프롬프트를 모두 상속 → 도메인별 메모리 격리 불가 → 컨텍스트 오염 발생.

**해결책**: 독립 에이전트 인스턴스를 Mattermost 채널을 통해 연결하는 구조.

```
[기존 서브에이전트 방식]
  부모 에이전트
    → 서브에이전트 (부모 메모리/모델/시스템프롬프트 모두 상속)
    → 컨텍스트 오염

[격리된 독립 에이전트 방식]
  프로젝트 에이전트 A (도메인 특화 메모리만)
       ↕ Mattermost 채널
  프로젝트 에이전트 B (별도 도메인 메모리)
       ↕ Mattermost 채널
  노드 에이전트 (시스템 어드민, 인프라만)
```

**이점**:
- 각 에이전트가 자신의 도메인 메모리만 축적
- 에이전트 간 셀프-리인포싱(서로 동의만 반복) 방지
- 사람(핸드폰)이 중간에 개입하기 용이
- 장기 암묵지: 과거 Claude Code 대화에서 시행착오·노하우 자동 추출 → 임베딩 벡터 저장
